...
import me.unknkriod.ang.AngApplication
...
import me.unknkriod.ang.dto.SubscriptionUpdateResult
import me.unknkriod.ang.dto.TestServiceMessage
import me.unknkriod.ang.extension.matchesPattern
...
import me.unknkriod.ang.handler.SpeedtestManager
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
     ...
     */
    fun testAllRealPing(serverGuids: List<String>? = null) {
        MessageUtil.sendMsg2TestService(
            getApplication(),
            TestServiceMessage(key = AppConfig.MSG_MEASURE_CONFIG_CANCEL)
        )
        val targetGuids = serverGuids ?: serversCache.map { it.guid }.toList()
        ...
        viewModelScope.launch(Dispatchers.Default) {
            ...
            MessageUtil.sendMsg2TestService(
                ...
                TestServiceMessage(
                    ...
                    subscriptionId = if (serverGuids == null) subscriptionId else "",
                    serverGuids = if (serverGuids != null || keywordFilter.isNotEmpty()) targetGuids else emptyList()
                )
            )
        }
    }
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_STATE_START_FAILURE -> {
                    ...
                    LogUtil.e(AppConfig.TAG, "V2Ray start failure: $errorMessage")
