...
import me.unknkriod.ang.extension.isNotNullEmpty
import me.unknkriod.ang.extension.nullIfBlank
...
import me.unknkriod.ang.dto.V2rayConfig
...
object CoreOutboundBuilder {
    ...
    fun convert(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
    ...
    fun createInitOutbound(configType: EConfigType): V2rayConfig.OutboundBean? {
    ...
    private fun toOutboundVmess(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
    ...
    private fun toOutboundVless(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
    ...
    private fun toOutboundShadowsocks(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
    ...
    private fun toOutboundTrojan(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
    ...
    private fun toOutboundSocks(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
    ...
    private fun toOutboundHttp(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
    ...
    private fun toOutboundWireguard(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
    ...
    private fun toOutboundHysteria2(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
     ...
     */
    fun populateTransportSettings(streamSettings: V2rayConfig.OutboundBean.StreamSettingsBean, profileItem: ProfileItem): String? {
...
//        val key = profileItem.quicKey
        ...
        when (streamSettings.network) {
            ...
            NetworkType.KCP.type -> {
                ...
                val udpMaskList = mutableListOf<V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean>()
                if (!headerType.isNullOrEmpty() && headerType != "none") {
                    ...
                    udpMaskList.add(
                        V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean(
                            type = kcpHeaderType,
                            settings = if (headerType == "dns" && !host.isNullOrEmpty()) {
                                V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean.MaskSettingsBean(
                                    domain = host
                                )
                            } else {
                                null
                ...
                if (seed.isNullOrEmpty()) {
                    ...
                    udpMaskList.add(
                        V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean(
                            type = "mkcp-aes128gcm",
                            settings = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean.MaskSettingsBean(
                                password = seed
                            )
                        )
                    )
                }
                streamSettings.finalmask = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean(
            ...
            NetworkType.HTTP_UPGRADE.type -> {
                val httpupgradeSetting = V2rayConfig.OutboundBean.StreamSettingsBean.HttpupgradeSettingsBean()
            ...
            NetworkType.XHTTP.type -> {
                val xhttpSetting = V2rayConfig.OutboundBean.StreamSettingsBean.XhttpSettingsBean()
...
//                    }
            ...
            NetworkType.GRPC.type -> {
                val grpcSetting = V2rayConfig.OutboundBean.StreamSettingsBean.GrpcSettingsBean()
            ...
            NetworkType.HYSTERIA.type -> {
                val hysteriaSetting = V2rayConfig.OutboundBean.StreamSettingsBean.HysteriaSettingsBean(
                    version = 2,
                    auth = profileItem.password.orEmpty(),
                )
                val quicParams = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.QuicParamsBean(
                ...
                if (profileItem.portHopping.isNotNullEmpty()) {
                    ...
                    quicParams.udpHop = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.QuicParamsBean.UdpHopBean(
                        ports = profileItem.portHopping,
                        interval = interval
                    )
                }
                val finalmask = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean(
     ...
     */
    fun populateTlsSettings(streamSettings: V2rayConfig.OutboundBean.StreamSettingsBean, profileItem: ProfileItem, sniExt: String?) {
     ...
     */
    private fun updateOutboundFragment(streamSettings: V2rayConfig.OutboundBean.StreamSettingsBean): Boolean {
        try {
            ...
            val fragmentMask = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean(
                type = "fragment",
                settings = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean.MaskSettingsBean(
                    packets = packets,
            ...
            val noiseMask = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean(
                type = "noise",
                settings = V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean.MaskSettingsBean(
                    noise = listOf(
                        V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean.MaskSettingsBean.NoiseMaskBean(
                            rand = "10-20",
                            delay = "10-16",
                        )
                    )
            ...
            fun prependMask(scope: String, mask: V2rayConfig.OutboundBean.StreamSettingsBean.FinalMaskBean.MaskBean) {
