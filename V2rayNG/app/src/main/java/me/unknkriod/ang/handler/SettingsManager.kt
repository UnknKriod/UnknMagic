...
import me.unknkriod.ang.AppConfig.ANG_PACKAGE
...
import me.unknkriod.ang.enums.Language
import me.unknkriod.ang.enums.RoutingType
import me.unknkriod.ang.enums.VpnInterfaceAddressConfig
...
object SettingsManager {
    ...
    fun routingRulesetsBypassLan(): Boolean {
        ...
        if (config.configType == EConfigType.CUSTOM) {
            ...
            val exist = v2rayConfig?.routing?.rules?.filter { it.outboundTag == AppConfig.TAG_DIRECT }?.any {
                it.domain?.contains(AppConfig.GEOSITE_PRIVATE) == true || it.ip?.contains(AppConfig.GEOIP_PRIVATE) == true
            }
            return exist == true
        }

        val rulesetItems = MmkvManager.decodeRoutingRulesets()
        val exist = rulesetItems?.filter { it.enabled && it.outboundTag == AppConfig.TAG_DIRECT }?.any {
            it.domain?.contains(AppConfig.GEOSITE_PRIVATE) == true || it.ip?.contains(AppConfig.GEOIP_PRIVATE) == true
        }
        return exist == true
    }
    ...
    fun removeSubscriptionWithDefault(subid: String) {
        ...
        val subsList2 = MmkvManager.decodeSubsList()
        ...
        MmkvManager.encodeSubscription(AppConfig.DEFAULT_SUBSCRIPTION_ID, defaultSub)
    }
    ...
    fun getLocale(): Locale {
        ...
        val language = Language.Companion.fromCode(langCode)
    ...
    fun getCurrentVpnInterfaceAddressConfig(): VpnInterfaceAddressConfig {
        ...
        return VpnInterfaceAddressConfig.Companion.getConfigByIndex(selectedIndex ?: 0)
    }
    ...
    fun isVpnMode(): Boolean {
        ...
        return mode == null || mode == AppConfig.VPN
    }
    ...
    private fun migrateServerListToSubscriptions() {
        ...
        guids.forEach { guid ->
            ...
            val subId = config.subscriptionId.ifEmpty { AppConfig.DEFAULT_SUBSCRIPTION_ID }
    ...
    private fun ensureHardcodedSubscription() {
        val hardcodedUrl = "https://gitverse.ru/api/repos/zieng2/wl/raw/branch/master/list_universal.txt"
        val hardcodedId = "hardcoded_sub"
        if (MmkvManager.decodeSubscription(hardcodedId) == null) {
            val hardcodedSub = SubscriptionItem(
                remarks = "Auto Subscription",
                url = hardcodedUrl
            )
            MmkvManager.encodeSubscription(hardcodedId, hardcodedSub)
        }
    }
