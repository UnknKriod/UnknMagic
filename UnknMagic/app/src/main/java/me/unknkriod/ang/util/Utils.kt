...
object Utils {
    ...
    fun tryDecodeBase64(text: String?): String? {
        ...
        try {
            return Base64.decode(trimmed, Base64.NO_WRAP).toString(Charsets.UTF_8)
        } catch (e: Exception) {
            // LogUtil.d(AppConfig.TAG, "Failed to decode standard base64")
        }
        try {
            return Base64.decode(trimmed, Base64.NO_WRAP.or(Base64.URL_SAFE)).toString(Charsets.UTF_8)
        } catch (e: Exception) {
            // LogUtil.d(AppConfig.TAG, "Failed to decode URL-safe base64")
        }
        return null
    }
    ...
    fun isValidSubUrl(value: String?): Boolean {
        ...
        try {
            ...
            if (URLUtil.isHttpUrl(value)) {
                if (value.contains(AppConfig.LOOPBACK)) return true
     ...
     */
    fun isXray(): Boolean = BuildConfig.APPLICATION_ID.startsWith("me.unknkriod.ang")
     ...
     */
    fun isGoogleFlavor(): Boolean = BuildConfig.FLAVOR == "premium"
