...
import me.unknkriod.ang.dto.AssetUrlCache
import me.unknkriod.ang.dto.AssetUrlItem
import me.unknkriod.ang.dto.ProfileItem
import me.unknkriod.ang.dto.RulesetItem
import me.unknkriod.ang.dto.ServerAffiliationInfo
import me.unknkriod.ang.dto.SubscriptionCache
import me.unknkriod.ang.dto.SubscriptionItem
import me.unknkriod.ang.dto.WebDavConfig
...
object MmkvManager {
    ...
    const val KEY_AUTO_MODE = "AUTO_MODE_ENABLED"
    ...
    fun decodeAllServerList(): MutableList<String> {
        ...
        if (!subsList.contains(AppConfig.DEFAULT_SUBSCRIPTION_ID)) {
            allServers.addAll(decodeServerList(AppConfig.DEFAULT_SUBSCRIPTION_ID))
        }
    ...
    /**
     * Decodes the recent servers list.
     */
    fun decodeRecentServers(): MutableList<String> {
        val json = mainStorage.decodeString(KEY_RECENT_SERVERS)
        return if (json.isNullOrBlank()) {
            mutableListOf()
        } else {
            JsonUtil.fromJson(json, Array<String>::class.java)?.toMutableList() ?: mutableListOf()
    ...
    /**
     * Encodes the recent servers list.
     */
    fun encodeRecentServers(recentServers: List<String>) {
        mainStorage.encode(KEY_RECENT_SERVERS, JsonUtil.toJson(recentServers))
    }

    /**
     * Adds a server to the recent servers list.
     */
    fun addRecentServer(guid: String) {
        if (guid.isBlank()) return
        val recentServers = decodeRecentServers()
        recentServers.remove(guid)
        recentServers.add(0, guid)
        if (recentServers.size > 3) { // Keep only last 3
            recentServers.removeAt(recentServers.size - 1)
        }
        encodeRecentServers(recentServers)
    }
    ...
    private fun getSubscriptionId(subscriptionId: String?): String {
        return subscriptionId?.ifEmpty { AppConfig.DEFAULT_SUBSCRIPTION_ID } ?: AppConfig.DEFAULT_SUBSCRIPTION_ID
    }
    ...
    fun decodeRoutingRulesets(): MutableList<RulesetItem>? {
        val ruleset = settingsStorage.decodeString(AppConfig.PREF_ROUTING_RULESET)
    ...
    fun encodeRoutingRulesets(rulesetList: MutableList<RulesetItem>?) {
        if (rulesetList.isNullOrEmpty())
            ...
            encodeSettings(AppConfig.PREF_ROUTING_RULESET, JsonUtil.toJson(rulesetList))
    }
