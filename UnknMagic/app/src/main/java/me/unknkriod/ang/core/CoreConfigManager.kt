...
import me.unknkriod.ang.dto.ConfigResult
import me.unknkriod.ang.dto.CoreConfigContext
...
import me.unknkriod.ang.util.PackageUidResolver
...
object CoreConfigManager {
    ...
    private fun getRoutingUserRule(configContext: CoreConfigContext, item: RulesetItem?, v2rayConfig: V2rayConfig) {
        ...
        rule.ip?.let { ipList ->
            ...
            ipList.forEach { ip ->
                when (ip) {
                    AppConfig.GEOIP_CN -> updatedIpList.add("geoip:cn")
                    AppConfig.GEOIP_PRIVATE -> updatedIpList.add("geoip:private")
